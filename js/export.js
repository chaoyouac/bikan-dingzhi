function exportDrawings() {
    'use strict';

    var params = window.getNicheParams ? window.getNicheParams() : {};
    var PIXELS_PER_MM = 0.8;
    var exportScale = 2;
    var fontSize = params.fontSize || 20;

    var w = params.width || 480;
    var h = params.totalHeight || 680;
    var fs = params.outerFrameSize || 20;
    var d = params.depth || 150;
    var fd = params.outerFrameDepth || 70;
    var lt = params.layerThickness || 10;
    var layers = params.layers || 3;
    var heights = params.layerHeights || [];
    var mode = params.outerFrameMode || 'uniform';

    var fsTop = mode === 'separate' ? (params.outerFrameTop || 0) : fs;
    var fsBottom = mode === 'separate' ? (params.outerFrameBottom || 0) : fs;
    var fsLeft = mode === 'separate' ? (params.outerFrameLeft || 0) : fs;
    var fsRight = mode === 'separate' ? (params.outerFrameRight || 0) : fs;
    var fdTop = mode === 'separate' ? (params.outerFrameTopDepth || 0) : fd;
    var fdBottom = mode === 'separate' ? (params.outerFrameBottomDepth || 0) : fd;
    var fdLeft = mode === 'separate' ? (params.outerFrameLeftDepth || 0) : fd;
    var fdRight = mode === 'separate' ? (params.outerFrameRightDepth || 0) : fd;

    var outerW = w + fsLeft + fsRight;
    var outerH = h + fsTop + fsBottom;
    var maxDepth = Math.max(d, fdTop, fdBottom, fdLeft, fdRight);

    var padTop = 60;
    var padBottom = 60;
    var padLeft = 90;
    var padRight = 90;
    var dimSpace = 45;
    var gap = 50;
    var infoPanelGap = 150;
    var infoPanelW = 500;

    var maxFrameSize = Math.max(fsTop, fsBottom, fsLeft, fsRight);
    var frontContentW = outerW + padLeft + padRight + dimSpace * 2;
    var frontContentH = outerH + padTop + padBottom + dimSpace * 2;
    var sideContentW = maxDepth + maxFrameSize + padLeft + padRight + dimSpace;
    var sideContentH = outerH + padTop + padBottom + dimSpace * 2;

    var totalContentW = frontContentW + gap + sideContentW + infoPanelGap / PIXELS_PER_MM + infoPanelW;
    var totalContentH = Math.max(frontContentH, sideContentH);

    var drawW = totalContentW * PIXELS_PER_MM * exportScale;
    var drawH = totalContentH * PIXELS_PER_MM * exportScale;

    var expCanvas = document.createElement('canvas');
    expCanvas.width = drawW;
    expCanvas.height = drawH;
    var expCtx = expCanvas.getContext('2d');
    expCtx.scale(exportScale, exportScale);

    redrawCombinedFixed(expCtx, params, drawW / exportScale, drawH / exportScale, PIXELS_PER_MM, fontSize);

    var cardH = 220;
    var pad = 30;
    var combinedW = Math.max(drawW, 900 * exportScale);
    var combinedH = drawH + pad * exportScale + cardH * exportScale;

    var finalCanvas = document.createElement('canvas');
    finalCanvas.width = combinedW;
    finalCanvas.height = combinedH;
    var ctx = finalCanvas.getContext('2d');

    ctx.fillStyle = '#ffffff';
    ctx.fillRect(0, 0, combinedW, combinedH);

    ctx.fillStyle = '#1a1a1a';
    ctx.font = 'bold ' + (22 * exportScale) + 'px "Microsoft YaHei", sans-serif';
    ctx.textAlign = 'center';
    var titlePrefix = params.type === 'surface' ? '明装' : '嵌入式';
    ctx.fillText(titlePrefix + '壁龛图纸', combinedW / 2, pad * exportScale / 2 + 16 * exportScale);

    var drawX = Math.floor((combinedW - drawW) / 2);
    ctx.drawImage(expCanvas, drawX, pad * exportScale);

    drawInfoCard(ctx, combinedW, combinedH - cardH * exportScale - pad * exportScale / 2, cardH * exportScale, pad * exportScale, params);

    var link = document.createElement('a');
    link.download = titlePrefix + '壁龛图纸.png';
    link.href = finalCanvas.toDataURL('image/png');
    link.click();
}

function redrawCombinedFixed(ctx, params, cw, ch, ppm, fontSize) {
    var w = params.width || 480;
    var h = params.totalHeight || 680;
    var fs = params.outerFrameSize || 20;
    var d = params.depth || 150;
    var fd = params.outerFrameDepth || 70;
    var lt = params.layerThickness || 10;
    var layers = params.layers || 3;
    var heights = params.layerHeights || [];
    var mode = params.outerFrameMode || 'uniform';

    var fsTop = mode === 'separate' ? (params.outerFrameTop || 0) : fs;
    var fsBottom = mode === 'separate' ? (params.outerFrameBottom || 0) : fs;
    var fsLeft = mode === 'separate' ? (params.outerFrameLeft || 0) : fs;
    var fsRight = mode === 'separate' ? (params.outerFrameRight || 0) : fs;
    var fdTop = mode === 'separate' ? (params.outerFrameTopDepth || 0) : fd;
    var fdBottom = mode === 'separate' ? (params.outerFrameBottomDepth || 0) : fd;
    var fdLeft = mode === 'separate' ? (params.outerFrameLeftDepth || 0) : fd;
    var fdRight = mode === 'separate' ? (params.outerFrameRightDepth || 0) : fd;

    var outerW = w + fsLeft + fsRight;
    var outerH = h + fsTop + fsBottom;
    var maxDepth = Math.max(d, fdTop, fdBottom, fdLeft, fdRight);

    var padTop = 60;
    var padBottom = 60;
    var padLeft = 90;
    var padRight = 90;
    var dimSpace = 45;
    var gap = 50;
    var infoPanelGap = 150;
    var infoPanelW = 500;

    var maxFrameSize = Math.max(fsTop, fsBottom, fsLeft, fsRight);
    var frontContentW = outerW + padLeft + padRight + dimSpace * 2;
    var frontContentH = outerH + padTop + padBottom + dimSpace * 2;
    var sideContentW = maxDepth + maxFrameSize + padLeft + padRight + dimSpace;
    var sideContentH = outerH + padTop + padBottom + dimSpace * 2;

    var totalContentW = frontContentW + gap + sideContentW + infoPanelGap / ppm + infoPanelW;
    var totalContentH = Math.max(frontContentH, sideContentH);

    var startX = -30;
    var startY = 100;
    var shiftX = 50;

    var frontX = startX + shiftX;
    var frontY = startY;
    var frontW = frontContentW * ppm;
    var frontH = totalContentH * ppm;

    var sideX = frontX + frontW + gap * ppm;
    var sideY = startY;
    var sideW = sideContentW * ppm;
    var sideH = totalContentH * ppm;

    // Labels at top (No right shift, move up 30px)
    ctx.fillStyle = '#cc3333';
    ctx.font = 'bold 18px "Microsoft YaHei", sans-serif';
    ctx.textAlign = 'center';
    var titlePrefix = params.type === 'surface' ? '明装' : '嵌入式';
    ctx.fillText(titlePrefix + '正面视图', startX + frontW / 2, startY - 45);
    ctx.fillText(titlePrefix + '侧面视图', startX + frontW + gap * ppm + sideW / 2, startY - 45);

    redrawFrontFixed(ctx, params, frontX, frontY, frontW, frontH, ppm, padTop, padBottom, padLeft, padRight, dimSpace, fontSize);
    redrawSideFixed(ctx, params, sideX, sideY, sideW, sideH, ppm, padTop, padBottom, padLeft, padRight, dimSpace, fontSize);
    
    // 信息面板显示在画布右侧，与画布同宽
    var infoX = cw - infoPanelW - 20;
    var infoY = startY;
    redrawInfoPanel(ctx, params, infoX, infoY, ppm, fontSize);
}

function redrawFrontFixed(ctx, params, areaX, areaY, areaW, areaH, ppm, padTop, padBottom, padLeft, padRight, dimSpace, fontSize) {
    var w = params.width || 480;
    var h = params.totalHeight || 680;
    var fs = params.outerFrameSize || 20;
    var layers = params.layers || 3;
    var lt = params.layerThickness || 10;
    var heights = params.layerHeights || [];
    var isSurface = params.type === 'surface';
    var mode = params.outerFrameMode || 'uniform';

    var fsTop = mode === 'separate' ? (params.outerFrameTop || 0) : fs;
    var fsBottom = mode === 'separate' ? (params.outerFrameBottom || 0) : fs;
    var fsLeft = mode === 'separate' ? (params.outerFrameLeft || 0) : fs;
    var fsRight = mode === 'separate' ? (params.outerFrameRight || 0) : fs;

    var outerW = w + fsLeft + fsRight;
    var outerH = h + fsTop + fsBottom;

    var drawX = areaX + padLeft * ppm;
    var drawY = areaY + padTop * ppm;

    var offsetX = drawX;
    var offsetY = drawY;

    function tx(x) { return offsetX + (x + outerW / 2) * ppm; }
    function ty(y) { return offsetY + (outerH / 2 - y) * ppm; }
    function sv(v) { return v * ppm; }

    if (!isSurface) {
        ctx.fillStyle = 'rgba(26,26,26,0.15)';
        ctx.strokeStyle = '#000000';
        ctx.lineWidth = 3;

        // Top border
        ctx.fillRect(tx(-outerW/2), ty(outerH/2), sv(outerW), sv(fsTop));
        ctx.strokeRect(tx(-outerW/2), ty(outerH/2), sv(outerW), sv(fsTop));

        // Bottom border
        ctx.fillRect(tx(-outerW/2), ty(-outerH/2 + fsBottom), sv(outerW), sv(fsBottom));
        ctx.strokeRect(tx(-outerW/2), ty(-outerH/2 + fsBottom), sv(outerW), sv(fsBottom));

        // Left border
        ctx.fillRect(tx(-outerW/2), ty(outerH/2 - fsTop), sv(fsLeft), sv(outerH - fsTop - fsBottom));
        ctx.strokeRect(tx(-outerW/2), ty(outerH/2 - fsTop), sv(fsLeft), sv(outerH - fsTop - fsBottom));

        // Right border
        ctx.fillRect(tx(outerW/2 - fsRight), ty(outerH/2 - fsTop), sv(fsRight), sv(outerH - fsTop - fsBottom));
        ctx.strokeRect(tx(outerW/2 - fsRight), ty(outerH/2 - fsTop), sv(fsRight), sv(outerH - fsTop - fsBottom));
    }

    var innerX = -outerW/2 + fsLeft;
    var innerY = outerH/2 - fsTop;

    ctx.fillStyle = 'rgba(26,26,26,0.06)';
    ctx.fillRect(tx(innerX), ty(innerY), sv(w), sv(h));
    ctx.strokeStyle = '#1a1a1a';
    ctx.lineWidth = isSurface ? 3 : 2;
    ctx.strokeRect(tx(innerX), ty(innerY), sv(w), sv(h));

    var accum = 0;
    for (var i = 0; i < layers - 1; i++) {
        accum += heights[i];
        var bt = innerY - accum;
        ctx.fillStyle = 'rgba(26,26,26,0.08)';
        ctx.fillRect(tx(innerX), ty(bt), sv(w), sv(lt));
        ctx.strokeStyle = '#1a1a1a';
        ctx.lineWidth = 2.5;
        ctx.strokeRect(tx(innerX), ty(bt), sv(w), sv(lt));
        accum += lt;
    }

    if (params.hasLight) {
        var lightH = 6;
        var boardThick = 1;
        var boardDepth = 10;
        // Top ceiling light board (1mm thick, 1cm deep, 5mm from back)
        ctx.fillStyle = '#555555';
        ctx.fillRect(tx(innerX), ty(innerY - boardThick / 2), sv(w), sv(boardThick));
        // LED strip on top ceiling
        var lightY = innerY - boardThick - lightH / 2;
        ctx.fillStyle = '#ffa726';
        ctx.fillRect(tx(innerX + 4), ty(lightY), sv(w - 8), sv(lightH));
        accum = 0;
        for (var k = 0; k < layers - 1; k++) {
            accum += heights[k] + lt;
            var ly = innerY - accum - lightH / 2;
            ctx.fillStyle = '#ffa726';
            ctx.fillRect(tx(innerX + 4), ty(ly), sv(w - 8), sv(lightH));
        }
    }

    var dimOff = 35 * ppm;

    ctx.strokeStyle = '#cc3333';
    ctx.lineWidth = 2;
    ctx.fillStyle = '#cc3333';
    ctx.font = 'bold ' + fontSize + 'px "Microsoft YaHei", sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'bottom';

    if (!isSurface) {
        var yTop = ty(outerH/2) - dimOff;
        ctx.beginPath();
        ctx.moveTo(tx(-outerW/2), yTop);
        ctx.lineTo(tx(outerW/2), yTop);
        ctx.moveTo(tx(-outerW/2), ty(outerH/2));
        ctx.lineTo(tx(-outerW/2), yTop);
        ctx.moveTo(tx(outerW/2), ty(outerH/2));
        ctx.lineTo(tx(outerW/2), yTop);
        ctx.stroke();
        ctx.fillText(outerW, (tx(-outerW/2)+tx(outerW/2))/2, yTop - 4);

        var xRight = tx(outerW/2) + dimOff;
        ctx.beginPath();
        ctx.moveTo(xRight, ty(outerH/2));
        ctx.lineTo(xRight, ty(-outerH/2));
        ctx.moveTo(tx(outerW/2), ty(outerH/2));
        ctx.lineTo(xRight, ty(outerH/2));
        ctx.moveTo(tx(outerW/2), ty(-outerH/2));
        ctx.lineTo(xRight, ty(-outerH/2));
        ctx.stroke();
        ctx.save();
        ctx.textAlign = 'left';
        ctx.textBaseline = 'middle';
        ctx.translate(xRight + 12, (ty(outerH/2)+ty(-outerH/2))/2);
        ctx.rotate(-Math.PI/2);
        ctx.fillText(outerH, 0, 0);
        ctx.restore();
    }

    var yBot = ty(-outerH/2) + dimOff;
    var xLeft = tx(-outerW/2) - dimOff;

    ctx.textAlign = 'center';
    ctx.textBaseline = 'top';

    if (mode === 'separate') {
        ctx.beginPath();
        ctx.moveTo(xLeft, ty(outerH/2));
        ctx.lineTo(xLeft, ty(innerY));
        ctx.moveTo(tx(-outerW/2), ty(outerH/2));
        ctx.lineTo(xLeft, ty(outerH/2));
        ctx.moveTo(tx(-outerW/2), ty(innerY));
        ctx.lineTo(xLeft, ty(innerY));
        ctx.stroke();
        ctx.save();
        ctx.textAlign = 'right';
        ctx.textBaseline = 'middle';
        ctx.translate(xLeft - 8, (ty(outerH/2) + ty(innerY))/2);
        ctx.rotate(-Math.PI/2);
        ctx.fillText(fsTop, 0, 0);
        ctx.restore();

        ctx.beginPath();
        ctx.moveTo(xLeft, ty(innerY));
        ctx.lineTo(xLeft, ty(innerY - h));
        ctx.moveTo(tx(innerX), ty(innerY));
        ctx.lineTo(xLeft, ty(innerY));
        ctx.moveTo(tx(innerX), ty(innerY - h));
        ctx.lineTo(xLeft, ty(innerY - h));
        ctx.stroke();
        ctx.save();
        ctx.textAlign = 'right';
        ctx.textBaseline = 'middle';
        ctx.translate(xLeft - 8, (ty(innerY) + ty(innerY - h))/2);
        ctx.rotate(-Math.PI/2);
        ctx.fillText(h, 0, 0);
        ctx.restore();

        ctx.beginPath();
        ctx.moveTo(xLeft, ty(innerY - h));
        ctx.lineTo(xLeft, ty(-outerH/2));
        ctx.moveTo(tx(-outerW/2), ty(innerY - h));
        ctx.lineTo(xLeft, ty(innerY - h));
        ctx.moveTo(tx(-outerW/2), ty(-outerH/2));
        ctx.lineTo(xLeft, ty(-outerH/2));
        ctx.stroke();
        ctx.save();
        ctx.textAlign = 'right';
        ctx.textBaseline = 'middle';
        ctx.translate(xLeft - 8, (ty(innerY - h) + ty(-outerH/2))/2);
        ctx.rotate(-Math.PI/2);
        ctx.fillText(fsBottom, 0, 0);
        ctx.restore();

        ctx.textAlign = 'center';
        ctx.textBaseline = 'top';

        ctx.beginPath();
        ctx.moveTo(tx(-outerW/2), yBot);
        ctx.lineTo(tx(innerX), yBot);
        ctx.moveTo(tx(-outerW/2), ty(-outerH/2));
        ctx.lineTo(tx(-outerW/2), yBot);
        ctx.moveTo(tx(innerX), ty(innerY - h));
        ctx.lineTo(tx(innerX), yBot);
        ctx.stroke();
        ctx.fillText(fsLeft, (tx(-outerW/2) + tx(innerX))/2, yBot + 6);

        ctx.beginPath();
        ctx.moveTo(tx(innerX), yBot);
        ctx.lineTo(tx(innerX + w), yBot);
        ctx.moveTo(tx(innerX), ty(innerY - h));
        ctx.lineTo(tx(innerX), yBot);
        ctx.moveTo(tx(innerX + w), ty(innerY - h));
        ctx.lineTo(tx(innerX + w), yBot);
        ctx.stroke();
        ctx.fillText(w, (tx(innerX) + tx(innerX + w))/2, yBot + 6);

        ctx.beginPath();
        ctx.moveTo(tx(innerX + w), yBot);
        ctx.lineTo(tx(outerW/2), yBot);
        ctx.moveTo(tx(innerX + w), ty(innerY - h));
        ctx.lineTo(tx(innerX + w), yBot);
        ctx.moveTo(tx(outerW/2), ty(-outerH/2));
        ctx.lineTo(tx(outerW/2), yBot);
        ctx.stroke();
        ctx.fillText(fsRight, (tx(innerX + w) + tx(outerW/2))/2, yBot + 6);
    } else {
        ctx.beginPath();
        ctx.moveTo(xLeft, ty(innerY));
        ctx.lineTo(xLeft, ty(innerY - h));
        ctx.moveTo(tx(innerX), ty(innerY));
        ctx.lineTo(xLeft, ty(innerY));
        ctx.moveTo(tx(innerX), ty(innerY - h));
        ctx.lineTo(xLeft, ty(innerY - h));
        ctx.stroke();
        ctx.save();
        ctx.textAlign = 'right';
        ctx.textBaseline = 'middle';
        ctx.translate(xLeft - 8, (ty(innerY) + ty(innerY - h))/2);
        ctx.rotate(-Math.PI/2);
        ctx.fillText(h, 0, 0);
        ctx.restore();

        ctx.textAlign = 'center';
        ctx.textBaseline = 'top';

        ctx.beginPath();
        ctx.moveTo(tx(innerX), yBot);
        ctx.lineTo(tx(innerX + w), yBot);
        ctx.moveTo(tx(innerX), ty(innerY - h));
        ctx.lineTo(tx(innerX), yBot);
        ctx.moveTo(tx(innerX + w), ty(innerY - h));
        ctx.lineTo(tx(innerX + w), yBot);
        ctx.stroke();
        ctx.fillText(w, (tx(innerX) + tx(innerX + w))/2, yBot + 6);
    }

    ctx.font = 'bold ' + fontSize + 'px "Microsoft YaHei", sans-serif';
    ctx.fillStyle = '#cc3333';
    ctx.textAlign = 'center';
    accum = 0;
    for (var j = 0; j < layers; j++) {
        var layerH = heights[j] || 0;
        var topY = innerY - accum;
        var midY = topY - layerH / 2;
        ctx.fillText('层高: ' + layerH, tx(innerX + w / 2), ty(midY) + 6);
        accum += layerH;
        if (j < layers - 1) accum += lt;
    }
}

function redrawSideFixed(ctx, params, areaX, areaY, areaW, areaH, ppm, padTop, padBottom, padLeft, padRight, dimSpace, fontSize) {
    var h = params.totalHeight || 680;
    var d = params.depth || 150;
    var fs = params.outerFrameSize || 20;
    var fd = params.outerFrameDepth || 70;
    var layers = params.layers || 3;
    var lt = params.layerThickness || 10;
    var heights = params.layerHeights || [];
    var wallThick = 1;
    var isSurface = params.type === 'surface';
    var mode = params.outerFrameMode || 'uniform';

    var fsTop = mode === 'separate' ? (params.outerFrameTop || 0) : fs;
    var fsBottom = mode === 'separate' ? (params.outerFrameBottom || 0) : fs;
    var fdTop = mode === 'separate' ? (params.outerFrameTopDepth || 0) : fd;
    var fdBottom = mode === 'separate' ? (params.outerFrameBottomDepth || 0) : fd;

    var outerH = h + fsTop + fsBottom;
    var maxDepth = Math.max(d, fdTop, fdBottom);

    var drawX = areaX + padLeft * ppm;
    var drawY = areaY + padTop * ppm;

    var offsetX = drawX;
    var offsetY = drawY;

    function tx(x) { return offsetX + x * ppm; }
    function ty(y) { return offsetY + (outerH / 2 - y) * ppm; }
    function sv(v) { return v * ppm; }

    var innerY = h / 2 - fsTop;

    ctx.fillStyle = 'rgba(26,26,26,0.06)';
    ctx.fillRect(tx(0), ty(innerY), sv(d), sv(h));
    ctx.strokeStyle = '#1a1a1a';
    ctx.lineWidth = isSurface ? 3 : 2;
    ctx.strokeRect(tx(0), ty(innerY), sv(d), sv(h));

    if (!isSurface) {
        ctx.lineWidth = 1.5;
        ctx.fillStyle = 'rgba(26,26,26,0.10)'; ctx.strokeStyle = '#1a1a1a';
        ctx.fillRect(tx(0), ty(h/2+fsTop), sv(wallThick), sv(fsTop));
        ctx.strokeRect(tx(0), ty(h/2+fsTop), sv(wallThick), sv(fsTop));
        ctx.fillRect(tx(0), ty(h/2+fsTop), sv(fdTop), sv(wallThick));
        ctx.strokeRect(tx(0), ty(h/2+fsTop), sv(fdTop), sv(wallThick));

        ctx.fillStyle = 'rgba(26,26,26,0.10)'; ctx.strokeStyle = '#1a1a1a';
        ctx.fillRect(tx(0), ty(-h/2), sv(wallThick), sv(fsBottom));
        ctx.strokeRect(tx(0), ty(-h/2), sv(wallThick), sv(fsBottom));
        ctx.fillRect(tx(0), ty(-h/2-fsBottom+wallThick), sv(fdBottom), sv(wallThick));
        ctx.strokeRect(tx(0), ty(-h/2-fsBottom+wallThick), sv(fdBottom), sv(wallThick));
    }

    var accum = 0;
    for (var j = 0; j < layers - 1; j++) {
        accum += heights[j];
        ctx.fillStyle = 'rgba(26,26,26,0.08)';
        ctx.fillRect(tx(0), ty(innerY-accum), sv(d), sv(lt));
        ctx.strokeStyle = '#1a1a1a';
        ctx.lineWidth = 2;
        ctx.strokeRect(tx(0), ty(innerY-accum), sv(d), sv(lt));
        accum += lt;
    }

    if (!isSurface) {
        var fdY = ty(h / 2 + fsTop) - 18 * ppm;
        ctx.strokeStyle = '#cc3333'; ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(tx(0), fdY);
        ctx.lineTo(tx(fdTop), fdY);
        ctx.moveTo(tx(0), ty(h/2+fsTop));
        ctx.lineTo(tx(0), fdY);
        ctx.moveTo(tx(fdTop), ty(h/2+fsTop));
        ctx.lineTo(tx(fdTop), fdY);
        ctx.stroke();
        ctx.fillStyle = '#cc3333'; ctx.font = 'bold ' + fontSize + 'px "Microsoft YaHei", sans-serif'; ctx.textAlign = 'center';
        ctx.textBaseline = 'bottom';
        ctx.fillText(fdTop, tx(fdTop/2), fdY - 6);

        var dx0 = tx(-12);
        ctx.strokeStyle = '#cc3333';
        ctx.fillStyle = '#cc3333';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(dx0, ty(h / 2 + fsTop));
        ctx.lineTo(dx0, ty(h / 2));
        ctx.moveTo(tx(0), ty(h / 2 + fsTop));
        ctx.lineTo(dx0, ty(h / 2 + fsTop));
        ctx.moveTo(tx(0), ty(h / 2));
        ctx.lineTo(dx0, ty(h / 2));
        ctx.stroke();
        ctx.save();
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.translate(dx0 - 14, (ty(h / 2 + fsTop) + ty(h / 2)) / 2);
        ctx.rotate(-Math.PI / 2);
        ctx.font = 'bold ' + fontSize + 'px "Microsoft YaHei", sans-serif';
        ctx.fillText(fsTop, 0, 0);
        ctx.restore();
    }

    var midY = ty(0);
    var dDimY = midY + 18 * ppm;
    ctx.strokeStyle = '#cc3333';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(tx(0), dDimY);
    ctx.lineTo(tx(d), dDimY);
    ctx.stroke();
    ctx.fillStyle = '#cc3333';
    ctx.font = 'bold ' + fontSize + 'px "Microsoft YaHei", sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(d, tx(d/2), dDimY + 20);
}

function redrawInfoPanel(ctx, params, x, y, ppm, fontSize) {
    var w = params.width || 480;
    var h = params.totalHeight || 680;
    var d = params.depth || 150;
    var fs = params.outerFrameSize || 20;
    var mode = params.outerFrameMode || 'uniform';
    var fsTop = mode === 'separate' ? (params.outerFrameTop || 0) : fs;
    var fsBottom = mode === 'separate' ? (params.outerFrameBottom || 0) : fs;
    var fsLeft = mode === 'separate' ? (params.outerFrameLeft || 0) : fs;
    var fsRight = mode === 'separate' ? (params.outerFrameRight || 0) : fs;
    var outerW = w + fsLeft + fsRight;
    var outerH = h + fsTop + fsBottom;
    var isSurface = params.type === 'surface';

    var lineH = fontSize * 1.8;
    var startX = x;
    var startY = y;

    ctx.fillStyle = '#cc3333';
    ctx.font = 'bold ' + fontSize + 'px "Microsoft YaHei", sans-serif';
    ctx.textAlign = 'left';

    var items = [];
    if (!isSurface) {
        items.push('外框尺寸（包含折边）：' + outerH + ' X ' + outerW);
    }
    items.push('内框尺寸（不包含折边）：' + h + ' X ' + w);
    items.push('深度：' + d);
    if (params.showDrawingNo && params.drawingNo) {
        items.push('编号：' + params.drawingNo);
    }
    if (params.showColor && params.color) {
        items.push('颜色：' + params.color);
    }
    if (params.showHasLight) {
        items.push('是否带灯：' + (params.hasLight ? '是' : '否'));
    }
    if (params.showDate && params.date) {
        items.push('日期：' + params.date);
    }
    if (params.showRemark && params.remark) {
        items.push('备注：' + params.remark);
    }

    for (var i = 0; i < items.length; i++) {
        ctx.fillText(items[i], startX, startY + i * lineH);
    }
}

function drawInfoCard(ctx, totalW, cardY, cardH, pad, params) {
    var cardW = totalW - pad * 2;
    var cardX = pad;
    var isSurface = params.type === 'surface';
    var mode = params.outerFrameMode || 'uniform';
    var fsTop = mode === 'separate' ? (params.outerFrameTop || 0) : params.outerFrameSize || 0;
    var fsBottom = mode === 'separate' ? (params.outerFrameBottom || 0) : params.outerFrameSize || 0;
    var fsLeft = mode === 'separate' ? (params.outerFrameLeft || 0) : params.outerFrameSize || 0;
    var fsRight = mode === 'separate' ? (params.outerFrameRight || 0) : params.outerFrameSize || 0;
    var outerH = (params.totalHeight || 680) + fsTop + fsBottom;
    var outerW = (params.width || 480) + fsLeft + fsRight;

    ctx.fillStyle = '#f8f9fa';
    ctx.strokeStyle = '#dee2e6';
    ctx.lineWidth = 1;
    ctx.beginPath();
    roundRectPoly(ctx, cardX, cardY, cardW, cardH, 8);
    ctx.fill();
    ctx.stroke();

    ctx.fillStyle = '#1a1a1a';
    ctx.font = 'bold 18px "Microsoft YaHei", sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText('参数信息 Parameters', cardX + 18, cardY + 30);

    ctx.strokeStyle = '#1a1a1a';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(cardX + 18, cardY + 38);
    ctx.lineTo(cardX + cardW - 18, cardY + 38);
    ctx.stroke();

    var items = [];
    if (!isSurface) {
        items.push(['外框尺寸（包含折边）', outerH + ' X ' + outerW + ' mm']);
    }
    items.push(['内框尺寸（不包含折边）', params.totalHeight + ' X ' + params.width + ' mm']);
    items.push(['深度', params.depth + ' mm']);
    if (params.showDrawingNo) {
        items.push(['编号', params.drawingNo || '']);
    }
    if (params.showColor) {
        items.push(['颜色', params.color || '']);
    }
    if (params.showHasLight) {
        items.push(['是否带灯', params.hasLight ? '是' : '否']);
    }
    if (params.showDate) {
        items.push(['日期', params.date || '']);
    }
    if (params.showRemark) {
        items.push(['备注', params.remark || '']);
    }

    var col1X = cardX + 18;
    var col2X = cardX + cardW / 2 + 8;
    var startY = cardY + 55;
    var lineH = 24;
    var half = Math.ceil(items.length / 2);

    ctx.font = '14px "Microsoft YaHei", sans-serif';
    for (var i = 0; i < half; i++) {
        ctx.fillStyle = '#666'; ctx.textAlign = 'left';
        ctx.fillText(items[i][0], col1X, startY + i * lineH);
        ctx.fillStyle = '#333';
        ctx.fillText(items[i][1], col1X + 180, startY + i * lineH);
    }
    for (var j = half; j < items.length; j++) {
        var idx = j - half;
        ctx.fillStyle = '#666'; ctx.textAlign = 'left';
        ctx.fillText(items[j][0], col2X, startY + idx * lineH);
        ctx.fillStyle = '#333';
        ctx.fillText(items[j][1], col2X + 180, startY + idx * lineH);
    }

    ctx.fillStyle = '#aaa';
    ctx.font = '11px "Microsoft YaHei", sans-serif';
    ctx.textAlign = 'right';
    ctx.fillText('Generated by 壁龛定制设计器', cardX + cardW - 18, cardY + cardH - 10);
}

function roundRectPoly(ctx, x, y, w, h, r) {
    ctx.beginPath();
    ctx.moveTo(x + r, y);
    ctx.lineTo(x + w - r, y);
    ctx.quadraticCurveTo(x + w, y, x + w, y + r);
    ctx.lineTo(x + w, y + h - r);
    ctx.quadraticCurveTo(x + w, y + h, x + w - r, y + h);
    ctx.lineTo(x + r, y + h);
    ctx.quadraticCurveTo(x, y + h, x, y + h - r);
    ctx.lineTo(x, y + r);
    ctx.quadraticCurveTo(x, y, x + r, y);
    ctx.closePath();
}

window.exportDrawings = exportDrawings;
