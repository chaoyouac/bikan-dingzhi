// OrbitControls - global script version (depends on THREE loaded globally)
(function() {
    const {
        EventDispatcher, MOUSE, Quaternion, Spherical, TOUCH,
        Vector2, Vector3, Plane, Ray, MathUtils
    } = THREE;

    if (!THREE) { console.error('THREE is required for OrbitControls'); return; }
    ...
    class OrbitControls extends EventDispatcher {
        constructor(object, domElement) {
            ...
            function getZoomScale(delta) {
                const nd = Math.abs(delta) / (100 * (window.devicePixelRatio | 0));
                return Math.pow(0.95, scope.zoomSpeed * nd);
            }
            ...
            function clampDistance(dist) { return Math.max(scope.minDistance, Math.min(scope.maxDistance, dist)); }

            function handleMouseDownRotate(event) { rotateStart.set(event.clientX, event.clientY); }
            function handleMouseDownDolly(event) { updateZoomParameters(event.clientX, event.clientX); dollyStart.set(event.clientX, event.clientY); }
            ...
            function handleTouchStartRotate(event) {
                ...
                else { const p = getSecondPointerPosition(event); rotateStart.set(0.5 * (event.pageX + p.x), 0.5 * (event.pageY + p.y)); }
            }
            function handleTouchStartPan(event) {
                if (pointers.length === 1) panStart.set(event.pageX, event.pageY);
                else { const p = getSecondPointerPosition(event); panStart.set(0.5 * (event.pageX + p.x), 0.5 * (event.pageY + p.y)); }
            }
            function handleTouchStartDolly(event) {
                const p = getSecondPointerPosition(event);
                dollyStart.set(0, Math.sqrt(Math.pow(event.pageX - p.x, 2) + Math.pow(event.pageY - p.y, 2)));
            }
            ...
            function handleTouchMoveRotate(event) {
                ...
                else { const p = getSecondPointerPosition(event); rotateEnd.set(0.5 * (event.pageX + p.x), 0.5 * (event.pageY + p.y)); }
                rotateDelta.subVectors(rotateEnd, rotateStart).multiplyScalar(scope.rotateSpeed);
                rotateLeft(2 * Math.PI * rotateDelta.x / scope.domElement.clientHeight);
                rotateUp(2 * Math.PI * rotateDelta.y / scope.domElement.clientHeight);
                rotateStart.copy(rotateEnd);
            ...
            function handleTouchMovePan(event) {
                ...
                else { const p = getSecondPointerPosition(event); panEnd.set(0.5 * (event.pageX + p.x), 0.5 * (event.pageY + p.y)); }
                panDelta.subVectors(panEnd, panStart).multiplyScalar(scope.panSpeed);
                pan(panDelta.x, panDelta.y);
                panStart.copy(panEnd);
            }
            ...
            function handleTouchMoveDolly(event) {
                const p = getSecondPointerPosition(event);
                const distance = Math.sqrt(Math.pow(event.pageX - p.x, 2) + Math.pow(event.pageY - p.y, 2));
                dollyEnd.set(0, distance);
                dollyDelta.set(0, Math.pow(dollyEnd.y / dollyStart.y, scope.zoomSpeed));
                dollyOut(dollyDelta.y);
                dollyStart.copy(dollyEnd);
                updateZoomParameters((event.pageX + p.x) * 0.5, (event.pageY + p.y) * 0.5);
            }
            ...
            function onMouseDown(event) {
                ...
                switch (mouseAction) {
                    ...
                    case MOUSE.ROTATE:
                        if (event.ctrlKey || event.metaKey || event.shiftKey) { if (scope.enablePan === false) return; handleMouseDownPan(event); state = STATE.PAN; }
                        else { if (scope.enableRotate === false) return; handleMouseDownRotate(event); state = STATE.ROTATE; }
                        break;
                    case MOUSE.PAN:
                        if (event.ctrlKey || event.metaKey || event.shiftKey) { if (scope.enableRotate === false) return; handleMouseDownRotate(event); state = STATE.ROTATE; }
                        else { if (scope.enablePan === false) return; handleMouseDownPan(event); state = STATE.PAN; }
                        break;
                ...
                switch (pointers.length) {
                    case 1:
                        switch (scope.touches.ONE) {
                            case TOUCH.ROTATE: if (scope.enableRotate === false) return; handleTouchStartRotate(event); state = STATE.TOUCH_ROTATE; break;
                            case TOUCH.PAN: if (scope.enablePan === false) return; handleTouchStartPan(event); state = STATE.TOUCH_PAN; break;
                            default: state = STATE.NONE;
                        }
                        break;
                    case 2:
                        switch (scope.touches.TWO) {
                            case TOUCH.DOLLY_PAN: if (scope.enableZoom === false && scope.enablePan === false) return; handleTouchStartDollyPan(event); state = STATE.TOUCH_DOLLY_PAN; break;
                            case TOUCH.DOLLY_ROTATE: if (scope.enableZoom === false && scope.enableRotate === false) return; handleTouchStartDollyRotate(event); state = STATE.TOUCH_DOLLY_ROTATE; break;
                            default: state = STATE.NONE;
                        }
                        break;
                    default: state = STATE.NONE;
            ...
            function addPointer(event) { pointers.push(event.pointerId); }
            function removePointer(event) { delete pointerPositions[event.pointerId]; for (let i = 0; i < pointers.length; i++) { if (pointers[i] == event.pointerId) { pointers.splice(i, 1); return; } } }
            function trackPointer(event) { let p = pointerPositions[event.pointerId]; if (p === undefined) { p = new Vector2(); pointerPositions[event.pointerId] = p; } p.set(event.pageX, event.pageY); }
            function getSecondPointerPosition(event) { return pointerPositions[event.pointerId === pointers[0] ? pointers[1] : pointers[0]]; }
